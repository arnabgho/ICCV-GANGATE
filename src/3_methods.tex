...
The patches are of three sizes (64$\times$64, 128$\times$128, 192$\times$192) and are placed at a random location in the image of size 256$\times$256 (refer \figref{fig:autocomplete_data_generation} for an example). In order to extend the technique beyond outlines we adopted the multistage procedure depicted in \figref{fig:simplified_edges} to generate human like sketches from images. We refer to these generated sketches as simplified edges.
...
\begin{figure*}[t]
    ...
    \caption{
    \vspace{-2mm}
    {\bf Conditioning variants for the Appearance Generator} \label{fig:arch-gate1}
    \vspace{-2mm}
    }
\end{figure*}
...
In detail, given an input feature tensor $X_l$, ResNet~\cite{he2016deep} maps it to
\begin{equation}
