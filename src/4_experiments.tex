...
\end{tabular} \\
\normalsize
    \caption{\textbf{Efficacy of outlines:} Networks trained on partial edges (PE) produce worse quality results than those trained on partial outlines (PO) (top). Similarly, an appearance synthesis model trained on full edges (FE) does not work as well as a network trained on full outlines (FO) when given outlines as an input (bottom).}
    \label{fig:ablation_partialedge_full_outline}
\end{figure}

As mentioned in~\secref{sec:shape}, we simulate partial strokes in our dataset by removing random square patches and use BicycleGAN~\cite{zhu2017toward} model for training. Few completion examples are shown in \figref{fig:autocomplete_generate}.
% \pd{Figure here}. 
...
\begin{table}[h]
    \centering
        ...
        \end{tabular}
    \caption{\label{table:2step_eval} {\textbf{Shape + Appearance Generation Evaluation:} PE Trained: trained from Partial Edges $\rightarrow$ Images; PO Trained: trained from Partial Outlines $\rightarrow$ Images; EC : Edge Completion ; FE Trained: trained on Full Edges; OC: Outline Completion; FO Trained: trained on Full Outlines } }
\end{table}
