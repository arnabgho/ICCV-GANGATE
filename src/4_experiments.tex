...
To explore the efficacy of our full pipeline, we introduce a new outline dataset, consisting of 200 images (150 train, 50 test) for each of 10 classes -- basketball, chicken, cookie, cupcake, moon, orange, soccer, strawberry,  watermelon and pineapple.\es{where are images coming from?}
In each image, we obtain rough outlines for the image. \ow{how?}
...
In addition, we compare our proposed gating strategy and SkinnyResNet architecture~\es{did we mention this name?} to the following methods for  conditional image generation: \begin{itemize}[noitemsep]
...
\figref{fig:alg_comp} shows the efficacy of Gating on the task outline to image generation. Naive conditioning techniques fail to properly condition the class and the Visual Turing Test in terms of Amazon Mechanical Turk's realism scores are shown in \figref{fig:acc_vs_real}.
...
\begin{tabular}{*{3}{c@{\hspace{3px}}}}
    ...
    \begin{subfigure}[t]{.30\linewidth}\caption{Partial Outline}\label{fig:basketball_partial}\end{subfigure} &
    \begin{subfigure}[t]{.30\linewidth}\caption{Trained on Partial Edges}\label{fig:basketball_full}\end{subfigure} &
    \begin{subfigure}[t]{.30\linewidth}\caption{Trained on Partial Outlines}\label{fig:soccer_partial}\end{subfigure} \\
    ...
    \begin{subfigure}[t]{.30\linewidth}\caption{Autocompleted Outline}\label{fig:soccer_full}\end{subfigure} &
    \begin{subfigure}[t]{.30\linewidth}\caption{Off the shelf trained on Edges}\label{fig:cupcake_partial}\end{subfigure} &
    \begin{subfigure}[t]{.30\linewidth}\caption{Trained on Outlines}\label{fig:cupcake_full}\end{subfigure}\\
\end{tabular} \\
    \caption{\textbf{Efficacy of using outlines:} In the first row the networks trained on partial edges and partial outlines are tested with small strokes provided by the user but fails to generate plausible results. The second row provides an autocompleted outline provided by the network and results on off the shelf network pretrained on edges and tested with the outline and the final image shows plausible generation using a network trained on outlines.}
    \label{fig:ablation_partial_full_outline}
    \vspace{-3mm}
\end{figure}

\paragraph{Why is training with outlines better than edges?}
