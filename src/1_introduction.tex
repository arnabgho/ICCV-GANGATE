...
We use sparse object outlines/sketches/simplified-edges instead of dense edge maps as the user input
\footnote{Our input is always a partial sketch/outline image or partial simplified-edge map. These terms are sometimes used interchangeably. One can think of partial sketch/outline as a sparsified form of simplified-edge map, which again can be thought of as a sparsified form of an edge map.}, as these are closer to the lines that novice users tend to draw~\cite{cole2008people}. 
Our model first completes the input, could be partial outline or partial simplified-edge, and then generates the image conditioned on the completed shape. There are several advantages to this two-stage approach.
