...
% \footnote{Our input is always a partial sketch/outline image or partial simplified-edge map. In the case of simple objects, as in our multi-class scenario, the outlines are the simplified edges.},
as these are closer to the lines that novice users tend to draw~\cite{cole2008people}. 
Our model first completes the input, which could be partial outlines or edges, and then generates the image conditioned on the completed shape. There are several advantages to this two-stage approach.
